...
public class labelComp {
	...
	static int textNum;
	static int panelNum;
	static int winWidth, windHeight;
...
//Approves file and reads through and adds it to one string that is then passed to the tokenizer object.
		...
		while (tokens.hasMoreTokens()) {
			...
			// Checks grammar for when to end the loop.
			...
			while (tokens.hasMoreTokens()) {
				list = tokens.nextToken();
				// Checks grammar for when to end the loop for inside and creating objects.
				if (list.equalsIgnoreCase("End;")) {
					break;
					// Checks grammar for when to create the window.
					...
					setWindow(list);
					list = tokens.nextToken();
					// Checks grammar for when to set the layout with the numbers provided.
					...
					// Checks grammar for when to create a textfield.
				} else if (list.equalsIgnoreCase("TextField")) {
					while (!list.contains(";")) {
						list = tokens.nextToken();
						textNum = Integer.parseInt(list.replaceFirst(";", ""));
					}
					// Checks grammar for when to create a panel.
					...
					// Checks grammar for when to create buttons.
				} else if (list.equalsIgnoreCase("Button")) {
					while (!list.contains(";")) {
						list = tokens.nextToken();
						buttons.add(list.replaceAll("\"|;", ""));
					}
					// Checks grammar for when to create labels.
				} else if (list.equalsIgnoreCase("Label")) {
					while (!list.contains(";")) {
						list = tokens.nextToken();
						labels.add(list.replaceAll("\"|;", ""));
					}
					// Checks grammar for when to create a group for radio buttons.
				} else if (list.equalsIgnoreCase("Group")) {
					list = tokens.nextToken();
					while (!list.contains(";")) {
						list = tokens.nextToken();
